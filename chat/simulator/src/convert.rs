...
impl SimSession {
    ...
    pub fn list(user: &SimUser, sessions: usize, events: usize) -> Vec<Self> {
        ...
        let range = Uniform::try_from(SESSION_START_OFFSET_DAYS).unwrap();
        ...
        rand_days
            ...
            .map(|days| {
                ...
                let end = start + Duration::minutes(rng.random_range(SESSION_LENGTH_MINUTES));
    ...
    pub fn to_analytics_events(self) -> Result<Vec<AnalyticsEventRow>> {
        ...
        for (i, event) in self.events.into_iter().enumerate() {
            ...
            row.server_ts = row.client_ts + rand::rng().random_range(SERVER_TS_MILLIS);
